...
- `data/*.test` — the vendored test files, **verbatim** from html5lib-tests.
  ...
  hand-edit or reformat them: they are pinned byte-for-byte to the commit recorded
  in `COMMIT`, and some of their whitespace is the actual test data (the whitespace
  cases). Use `update.py` to change them.
...
It re-downloads `data/*.test` (and `LICENSE`) at the resolved commit, records it in
`COMMIT`, and reports which files it added, updated, removed, or left unchanged. If
new upstream cases fail, decide consciously: fix the serializer (a real find), tune
the load-time filter, or exclude the case with a note.
